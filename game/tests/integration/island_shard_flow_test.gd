...
func run(support: TestSupport, scene_tree: SceneTree) -> void:
	...
	support.expect(world.hud.island_install_button.get_node(world.hud.island_install_button.focus_neighbor_top) == world.hud.island_slot_previous_button, "controller focus must navigate from Install to world-slot selection")
	...
	world._apply_island_modifiers()
	support.expect(world.installed_shard.id == shard.id, "reapplying island modifiers must retain the installed stable identity")
	...
	support.expect(world.hud.select_island(0) and world.install_selected_shard(0), "selected Tempest shard must replace Verdant")
	...
	support.expect(world.install_selected_shard(2) and world.island_slot.shard_biome == "frozen", "reinstall must restore Tempest behavior and physical biome identity")
	...
	await scene_tree.process_frame
	support.expect(restored.load_game(path), "current save with island state must load")
