...
func _run() -> void:
	...
	if suite in ["all", "unit"]:
		for definition: Dictionary in [
			...
			{"path": "res://game/tests/unit/equipment_inventory_test.gd", "assertions": 11},
			...
			if test != null:
				...
				test.run(support)
				if not support.require_assertion_count(support.assertions - before, int(definition.assertions), path):
					_finish(suite)
					return
	...
	if suite in ["all", "simulation"]:
		var simulation_test: Variant = _instantiate_test("res://game/tests/simulation/first_playable_smoke_test.gd")
		if simulation_test != null:
			var smoke_before := support.assertions
			var metrics: Dictionary = await simulation_test.run(support, self)
			if not support.require_assertion_count(support.assertions - smoke_before, 5, "res://game/tests/simulation/first_playable_smoke_test.gd"):
				_finish(suite)
				return
			print("SMOKE_METRICS %s" % JSON.stringify(metrics))
		var rift_simulation: Variant = _instantiate_test("res://game/tests/simulation/rift_repeat_simulation_test.gd")
		if rift_simulation != null:
			...
			if not support.require_assertion_count(support.assertions - rift_before, 5, "res://game/tests/simulation/rift_repeat_simulation_test.gd"):
				_finish(suite)
				return
