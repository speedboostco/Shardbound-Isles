...
func run(support: TestSupport, scene_tree: SceneTree) -> Dictionary:
	...
	support.expect(metrics.get("gathered_wood") == 5 and metrics.get("wood") == 0, "smoke scenario must explore two caches, gather deterministic wood, and invest it in opening technologies")
	support.expect(metrics.get("gathered_stone") == 4 and metrics.get("stone") == 1, "smoke scenario must explore two caches, gather deterministic stone, and invest most of it in opening technologies")
