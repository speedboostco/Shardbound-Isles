...
func run(support: TestSupport) -> void:
	...
	for profile_id: String in ["player", "enemy", "ranged_enemy", "resident", "resource", "obstacle", "living_prop"]:
		...
		support.expect(float(profile.get("foot_y", 0.0)) >= 0.0 and float(profile.get("foot_y", 0.0)) <= 10.0, "%s shadow must overlap the authored foot baseline without an air gap" % profile_id)
