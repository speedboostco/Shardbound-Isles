...
func run(support: TestSupport) -> void:
	...
	support.expect(SaveService.SCHEMA_VERSION == 11, "island story saves must use schema version 11")
	support.expect(bool(decoded.ok), "schema-7 archipelago state must round trip")
...
func _state(archipelago: Dictionary, inventory: Array[Dictionary]) -> Dictionary:
	return {
		"player": {"health": 10, "maximum_health": 10, "mana": 60.0, "maximum_mana": 60.0, "mana_regeneration": 6.0, "position": {"x": 0.0, "y": 0.0}},
		"wood": 0,
		"stone": 0,
		"moonleaf": 0,
		"plank": 0,
		...
		"survival": SurvivalPreparationScript.default_state(),
		"journey": JourneyJournalScript.default_state(),
		"expedition": {"world_seed": 73000, "elapsed_seconds": 0.0, "shelter_remaining": 0.0},
		"forage": {"fiber_west": {"available": true, "regrow_remaining": 0.0}, "fiber_south": {"available": true, "regrow_remaining": 0.0}, "emberberry_north": {"available": true, "regrow_remaining": 0.0}, "emberberry_east": {"available": true, "regrow_remaining": 0.0}},
		"expedition_marks": 0,
		"expedition_contract": ExpeditionContractScript.default_state(),
		"island_story": IslandStoryQuestScript.default_state(),
	}
