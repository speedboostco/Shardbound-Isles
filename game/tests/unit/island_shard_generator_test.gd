...
func run(support: TestSupport) -> void:
	...
	support.expect(ember.get("id") == "emberglass_reach_9002" and ember.get("biome") == "ember", "second seed must select stable Emberglass identity")
	...
	support.expect(tempest.get("id") == "tempest_loom_9003" and tempest.get("biome") == "tempest", "third seed must select stable Tempest identity")
	...
	support.expect(IslandShardGenerator.generate(9004).get("id") == "verdant_crucible_9004", "definition selection must cycle deterministically")
