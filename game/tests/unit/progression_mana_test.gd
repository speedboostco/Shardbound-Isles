...
func run(support: TestSupport) -> void:
	...
	support.expect(definitions.size() == 7, "the visual technology tree exposes seven authored nodes")
	...
	support.expect(RecipeRegistry.all().size() == 13 and RecipeRegistry.validate().is_empty(), "all thirteen production recipes must validate")
