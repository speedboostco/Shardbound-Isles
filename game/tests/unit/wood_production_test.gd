...
const ProductionScript := preload("res://game/core/wood_production.gd")
...
func run(support: TestSupport) -> void:
	...
	var accelerated: Variant = ProductionScript.new()
	accelerated.set_interval_multiplier(0.5)
	accelerated.advance(2.0)
	support.expect(accelerated.stored_wood == 2, "half interval multiplier must produce twice per base interval")
	accelerated.restore(0)
	accelerated.set_interval_multiplier(1.0)
	accelerated.advance(2.0)
	support.expect(accelerated.stored_wood == 1, "reset interval multiplier must restore baseline cadence")
