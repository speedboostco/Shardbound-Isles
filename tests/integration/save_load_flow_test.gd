...
func run(support: TestSupport, scene_tree: SceneTree) -> void:
	...
	support.expect(restored.wood == 5 and restored.equipment_inventory.scrap == 2, "load must restore resources")
	...
	support.expect(restored.equipment_inventory.equipped_id == String(item.id) and restored.player.attack_damage == 6, "load must restore equipped ID and derived damage")
	support.expect(restored.reinforced_heart_crafted and restored.tidecatcher_built, "load must restore progression and construction")
